...
pub(crate) fn add_headers(row: &mut Vec<Cell>, total_bytes_sent: u64, total_bytes_recv: u64, elapsed: u64) {
    row.push(Cell::new(RHS, "in"));
    row.push(Cell::new(RHS, ":"));
    row.push(Cell::new(RHS, &speed(total_bytes_recv, elapsed)));
    row.push(Cell::new(RHS, ""));
    row.push(Cell::new(LHS, ""));
    row.push(Cell::new(LHS, ""));
    row.push(Cell::new(RHS, "out"));
    row.push(Cell::new(RHS, ":"));
    row.push(Cell::new(RHS, &speed(total_bytes_sent, elapsed)));
    row.push(Cell::new(LHS, ""));
    row.push(Cell::new(LHS, ""));
}

pub(crate) fn add(row: &mut Vec<Cell>, stream: &PacStream, total_bytes_sent: u64, total_bytes_recv: u64, elapsed: u64) {
    row.push(Cell::new(RHS, &pct_fmt(stream.bytes_recv_last as f64 / total_bytes_recv as f64)));
    row.push(Cell::new(RHS, " "));
    row.push(Cell::new(RHS, &speed(stream.bytes_recv_last, elapsed)));
    row.push(Cell::new(RHS, " ("));
    ...
    row.push(Cell::new(RHS, " "));
    row.push(Cell::new(RHS, &speed(stream.bytes_sent_last, elapsed)));
    row.push(Cell::new(RHS, " ("));
